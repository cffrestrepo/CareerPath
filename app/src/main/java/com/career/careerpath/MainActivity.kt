...
class MainActivity : AppCompatActivity() {
    ...
    /*
        ...
        val firebaseFirestorage  = FirebaseFirestore.getInstance()

        for(i in 0 until jsonArr.length()){
            val aux = jsonArr.get(1) as JSONObject
            val speaker = Speaker()
            ...
            firebaseFirestorage.collection("speakers").document().set(speaker)
        }


        for(i in 0 until jsonArr2.length()){
            val aux = jsonArr2.get(1) as JSONObject
            ...
            firebaseFirestorage.collection("conferences").document().set(conference)
        }
*/
    //endregion
